...
[![Install](https://img.shields.io/visual-studio-marketplace/i/AnhNT.claude-commit-ai?style=for-the-badge&logo=visualstudiocode&label=Installs)](https://marketplace.visualstudio.com/items?itemName=AnhNT.claude-commit-ai)
[![Version](https://img.shields.io/visual-studio-marketplace/v/AnhNT.claude-commit-ai?style=for-the-badge&logo=visualstudiocode)](https://marketplace.visualstudio.com/items?itemName=AnhNT.claude-commit-ai)
[![Rating](https://img.shields.io/visual-studio-marketplace/r/AnhNT.claude-commit-ai?style=for-the-badge&logo=visualstudiocode)](https://marketplace.visualstudio.com/items?itemName=AnhNT.claude-commit-ai)
[![License](https://img.shields.io/github/license/theanhnb97/claude-commit?style=for-the-badge)](LICENSE)

**Already using Claude Code? Get the commit message button you deserve â€“ at no extra cost.**

A VS Code extension that brings the âœ¨ sparkle button to your Git panel, powered by the Claude CLI you already have. Generate intelligent commit messages without paying for additional AI services.

![demo](demo.png)

## Why Claude Commit?

You're already investing in Claude Code â€“ whether it's Pro, Max Ã—5, or Max Ã—10. Why pay for Copilot or Cursor just for commit message generation? This extension leverages your existing Claude subscription to bring the same AI-powered commit message functionality directly to VS Code.

**Zero additional cost. Zero complexity. Just works.**

## Features

- **One-click commit message generation**: The sparkle button âœ¨ you know and love, right in VS Code's Source Control panel
- **Powered by your Claude CLI**: Uses your existing Claude installation â€“ no extra API keys or subscriptions needed
...
- **No conflicts**: Leaving `commitGenAI.cliPath` empty allows automatic detection in both environments
...
4. Look for the sparkle âœ¨ button in your Source Control panel
...
2. Stage your changes (optional â€“ works with unstaged changes too)
3. Click the sparkle âœ¨ button in the Source Control panel
...
* `commitGenAI.cliPath`: Path to Claude CLI executable (leave empty for auto-detection)
* `commitGenAI.anthropicApiKey`: Your Anthropic API key (`ANTHROPIC_API_KEY`, only needed if using API method)
* `commitGenAI.baseUrl`: Custom Claude API base URL (`ANTHROPIC_BASE_URL`, leave empty for Anthropic default endpoint)
* `commitGenAI.apiProvider`: API provider to use (`anthropic` or `openai-responses`) – defaults to `anthropic`
* `commitGenAI.openAiApiKey`: OpenAI-compatible API key (`OPENAI_API_KEY`, only used when apiProvider is `openai-responses`)
* `commitGenAI.openAiBaseUrl`: OpenAI-compatible API base URL (`OPENAI_BASE_URL`) – defaults to `https://api.openai.com/v1`
* `commitGenAI.openAiModel`: Model name for OpenAI-compatible Responses API – defaults to `gpt-4.1-mini`
* `commitGenAI.apiKey`: Legacy Anthropic API key setting (still supported)
* `commitGenAI.preferredMethod`: Choose AI backend (`auto`, `cli`, or `api`) â€“ defaults to `auto`
* `commitGenAI.model`: Claude model to use (`haiku`, `sonnet`, `opus`, `custom`) â€“ defaults to `haiku`
* `commitGenAI.customModelName`: Custom model name used when `commitGenAI.model` is `custom`
* `commitGenAI.language`: Commit message language (`en` for English, `ko` for Korean, `ua` for Ukrainian, `zh` for Chinese) â€“ defaults to `en`
* `commitGenAI.multiLineCommit`: Generate detailed multi-line commits with body and footer â€“ defaults to `false`
* `commitGenAI.diffSource`: Which changes to use (`staged`, `all`, or `auto`) â€“ defaults to `auto`
* `commitGenAI.claudeCodeManaged`: Let Claude Code generate commit messages with minimal intervention (only works with CLI method) â€“ defaults to `false`
* `commitGenAI.keepCoAuthoredBy`: Keep Co-Authored-By signature in commit message (only works in Claude Code managed mode) â€“ defaults to `false`
* `commitGenAI.messageAutoCloseSeconds`: Auto-close timeout for success message in seconds (0 to disable) â€“ defaults to `5`
* `commitGenAI.privacyMode`: Restrict temporary prompt file permissions to owner-only on Linux/macOS â€“ defaults to `false`
* `commitGenAI.commitStyle`: Commit message style format (`conventional`, `prefix`, `default`, or `custom`) â€“ defaults to `conventional`
* `commitGenAI.customPromptTemplate`: Custom prompt template with `{diff}` and `{stats}` variables (only used when commitStyle is `custom`) â€“ defaults to empty
...
```json
{
    "commitGenAI.preferredMethod": "auto"
}
```
...
```json
{
    "commitGenAI.cliPath": "/Users/you/.nvm/versions/node/v22.13.0/bin/claude",
    "commitGenAI.preferredMethod": "cli"
}
```
...
{
    "commitGenAI.preferredMethod": "auto",
    "commitGenAI.anthropicApiKey": "your-api-key-here"
}
```

### Using a custom Claude API endpoint
```json
{
    "commitGenAI.preferredMethod": "api",
    "commitGenAI.anthropicApiKey": "your-api-key-here",
    "commitGenAI.baseUrl": "https://your-claude-compatible-endpoint.example.com"
}
```

### Using OpenAI-compatible Responses API
```json
{
    "commitGenAI.preferredMethod": "api",
    "commitGenAI.apiProvider": "openai-responses",
    "commitGenAI.openAiApiKey": "your-api-key-here",
    "commitGenAI.openAiBaseUrl": "https://api.openai.com/v1",
    "commitGenAI.openAiModel": "gpt-4.1-mini"
}
```

For OpenAI-compatible gateways, set `openAiBaseUrl` to the provider's `/v1` base URL. The extension calls the `/responses` endpoint.

### Korean interface
```json
{
    "commitGenAI.language": "ko"
}
```
...
```json
{
    "commitGenAI.multiLineCommit": true
}
```
...
```json
{
    "commitGenAI.model": "sonnet"
}
```
Available models: `haiku` (fast, default), `sonnet` (balanced), `opus` (most capable), `custom` (use your own model name).

### Using a custom model name
```json
{
    "commitGenAI.model": "custom",
    "commitGenAI.customModelName": "claude-your-custom-model"
}
```
...
```json
{
    "commitGenAI.claudeCodeManaged": true,
    "commitGenAI.preferredMethod": "cli"
}
```
...
{
    "commitGenAI.claudeCodeManaged": true,
    "commitGenAI.keepCoAuthoredBy": true,
    "commitGenAI.preferredMethod": "cli"
}
```
...
```json
{
    "commitGenAI.messageAutoCloseSeconds": 10
}
```
...
```json
{
    "commitGenAI.commitStyle": "conventional"
}
```
...
```json
{
    "commitGenAI.commitStyle": "prefix"
}
```
...
```json
{
    "commitGenAI.commitStyle": "default"
}
```
...
```json
{
    "commitGenAI.commitStyle": "custom",
    "commitGenAI.customPromptTemplate": "Generate a concise commit message following our team's style.\n\nChanges:\n{stats}\n\nDiff (first 6000 chars):\n{diff}\n\nFormat: <category>: <description>\nCategories: feature, bugfix, refactor, docs, test, chore\nReturn only the commit message."
}
```
...
3. **Configure the path in settings** (only if needed):
   ...
   - Search for "commitGenAI.cliPath"
...
6. **Common issues**:
   - **WSL users**: Keep `commitGenAI.cliPath` empty to allow runtime detection in both Windows and WSL
...
3. Try using API method as fallback:
   ...
   - Add to settings: `commitGenAI.anthropicApiKey`
   - Set `commitGenAI.preferredMethod` to `api`

### Extension not working

1. Check you're in a Git repository
2. **Open Output panel** (View â†’ Output) and select **"Claude Commit"** from the dropdown
...
5. If CLI path is incorrect, clear `commitGenAI.cliPath` in settings to trigger re-detection
...
4. **Common causes**:
   ...
   - **Permissions**: Try setting `commitGenAI.privacyMode: false`
...
Found a bug or have a feature request? Please open an issue on our [GitHub repository](https://github.com/theanhnb97/claude-commit).
...
**Stop paying twice for AI commit messages. You've got Claude Code â€“ now get the commit button. âœ¨**
