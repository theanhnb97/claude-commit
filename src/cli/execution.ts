...
// call â€” they slow generation and steer output away from the commit message.
...
type ApiProvider = "anthropic" | "openai-responses";
...
function getClaudeApiKey(config: vscode.WorkspaceConfiguration): string | undefined {
  return (
...
function getClaudeBaseUrl(config: vscode.WorkspaceConfiguration): string | undefined {
  return config.get<string>("baseUrl")?.trim() || process.env.ANTHROPIC_BASE_URL?.trim();
}

function getApiProvider(config: vscode.WorkspaceConfiguration): ApiProvider {
  return config.get<ApiProvider>("apiProvider", "anthropic");
}

function getOpenAIApiKey(config: vscode.WorkspaceConfiguration): string | undefined {
  return config.get<string>("openAiApiKey")?.trim() || process.env.OPENAI_API_KEY?.trim();
}

function getOpenAIBaseUrl(config: vscode.WorkspaceConfiguration): string {
  return (
    config.get<string>("openAiBaseUrl")?.trim() || process.env.OPENAI_BASE_URL?.trim() || "https://api.openai.com/v1"
  );
}

function getOpenAIModel(config: vscode.WorkspaceConfiguration): string {
  const model = config.get<string>("openAiModel")?.trim();

  if (!model) {
    throw new Error("OpenAI-compatible provider selected but commitGenAI.openAiModel is empty.");
  }

  return model;
}

function getClaudeModel(config: vscode.WorkspaceConfiguration): string {
  const modelSetting = config.get<Model>("model", "haiku");

  if (modelSetting === "custom") {
    const customModelName = config.get<string>("customModelName")?.trim();

    if (!customModelName) {
      throw new Error("Custom model selected but commitGenAI.customModelName is empty.");
    }
...
function getClaudeApiModel(config: vscode.WorkspaceConfiguration): string {
  const model = getClaudeModel(config);
  const modelMap: Record<Exclude<Model, "custom">, string> = {
    haiku: "claude-haiku-4-5-20251001",
    sonnet: "claude-sonnet-4-6",
    opus: "claude-opus-4-6",
  };

  return modelMap[model as Exclude<Model, "custom">] ?? model;
}
...
export async function generateWithCLIManaged(
  ...
  // prompt cannot be quoted safely â€” skipped there, as before.
...
function extractOpenAIResponseText(response: OpenAIResponseBody): string {
  ...
  throw new Error("OpenAI-compatible API returned no output text.");
}
...
async function generateWithOpenAIResponsesAPI(
  ...
  if (!apiKey) {
    throw new Error("OPENAI_API_KEY not found. Set commitGenAI.openAiApiKey or environment variable.");
  }

  const model = getOpenAIModel(config);

  if (progressCallback) {
    progressCallback(`Connecting to OpenAI-compatible Responses API (${model})...`);
  }

  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const OpenAI = require("openai");
    const client = new OpenAI({ apiKey, baseURL: getOpenAIBaseUrl(config) });
    ...
    return stripCodeFences(extractOpenAIResponseText(response as OpenAIResponseBody));
  } catch (error) {
    const err = error as Error & { code?: string; status?: number };
    if (err.code === "MODULE_NOT_FOUND") {
      throw new Error("Install openai to use OpenAI-compatible Responses API: npm install openai");
    }
    if (err.status === 401) {
      throw new Error("Invalid OpenAI-compatible API key. Check commitGenAI.openAiApiKey or OPENAI_API_KEY.");
    }
    if (err.status === 429) {
...
async function generateWithAnthropicAPI(
  ...
  const apiKey = getClaudeApiKey(config);
  ...
  const apiModel = getClaudeApiModel(config);
...
export async function generateWithAPI(
  ...
  if (getApiProvider(config) === "openai-responses") {
    return generateWithOpenAIResponsesAPI(prompt, config, progressCallback);
  }

  return generateWithAnthropicAPI(prompt, config, progressCallback);
}
